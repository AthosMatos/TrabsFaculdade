...
void ModelClass::updateModel(string ModelName)
{
	if (ModelName == PreloadCube) loadPreloadedCube();
	else
	{
...
void ModelClass::loadObj()
{
	...
	bool res = loadOBJ("legoobj.obj", tempM->vertices, uvs, normals);
	// Load it into a VBO

	
	glGenBuffers(1, &tempM->vertexbuffer);
	glBindBuffer(GL_ARRAY_BUFFER, tempM->vertexbuffer);
	glBufferData(GL_ARRAY_BUFFER, tempM->vertices.size() * sizeof(glm::vec3), &tempM->vertices[0], GL_STATIC_DRAW);
	...
	glBindBuffer(GL_ARRAY_BUFFER, tempM->normalbuffer);
	...
	Models.insert(pair<string, Model*>("ObjLoaded", tempM));
}

void ModelClass::UpdateObj()
{
	// Clear the screen
	glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);

	// Use our shader
	glUseProgram(Models["ObjLoaded"]->programID);
	float size = 0.05f;
	...
	// in the "MVP" uniform
	glUniformMatrix4fv(Models["ObjLoaded"]->MatrixID, 1, GL_FALSE, &MVP[0][0]);
	glUniformMatrix4fv(Models["ObjLoaded"]->ModelMatrixID, 1, GL_FALSE, &ModelMatrix[0][0]);
	glUniformMatrix4fv(Models["ObjLoaded"]->ViewMatrixID, 1, GL_FALSE, &ViewMatrix[0][0]);

	glm::vec3 lightPos = glm::vec3(4, -20, 4);
	glUniform3f(Models["ObjLoaded"]->LightID, lightPos.x, lightPos.y, lightPos.z);

	// Bind our texture in Texture Unit 0
	glActiveTexture(GL_TEXTURE0);
	glBindTexture(GL_TEXTURE_2D, Models["ObjLoaded"]->Texture);
	// Set our "myTextureSampler" sampler to use Texture Unit 0
	glUniform1i(Models["ObjLoaded"]->TextureID, 0);

	// 1rst attribute buffer : vertices
	glEnableVertexAttribArray(0);
	glBindBuffer(GL_ARRAY_BUFFER, Models["ObjLoaded"]->vertexbuffer);
	...
	glBindBuffer(GL_ARRAY_BUFFER, Models["ObjLoaded"]->uvbuffer);
	glVertexAttribPointer(
		1,                                // attribute
		2,                                // size
		GL_FLOAT,                         // type
	...
	glBindBuffer(GL_ARRAY_BUFFER, Models["ObjLoaded"]->normalbuffer);
	glVertexAttribPointer(
		2,                                // attribute
		3,                                // size
		GL_FLOAT,                         // type
...
void ModelClass::RenderObj()
{
	// Draw the triangles !
	glDrawArrays(GL_TRIANGLES, 0, Models["ObjLoaded"]->vertices.size());
